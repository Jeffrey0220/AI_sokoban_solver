...
# largest_weight=max(self.weights)
        # largest_weight_i=self.weights.index(largest_weight)

        # largest_weight_box=n.state[1:][largest_weight_i]

        # if largest_weight_box not in self.targets:
        #     distances=[]
        #     b_x,b_y=largest_weight_box

        #     for i in range(len(self.targets)):
        #         t_x,t_y=self.targets[i]
        #         distances.append(abs(b_x-t_x)+abs(b_y-t_y))
            
        #     return min(distances)*largest_weight
...
import functools
import search
from mySokobanSolver import solve_weighted_sokoban, check_elem_action_seq,SokobanPuzzle
...
if __name__ == "__main__":
    ...
    wh.load_warehouse("./warehouses/test10.txt")#warehouse_8a#warehouse_91,c45,t350#
    print(wh.worker)
    print(wh.boxes)
    print(type(wh.worker))
    print(f"boxes={wh.boxes}")
    print([wh.worker]+wh.boxes)
    print(tuple([wh.worker]+wh.boxes))
    print(wh.walls)
    print(wh.targets)
    print(f"weight={wh.weights},{wh.weights[1]}")
    print(wh.ncols)
    print(wh.nrows)
    print(len(wh.weights)==len(wh.boxes))

    print(wh)   # this calls    wh.__str__()
    res=check_elem_action_seq(wh,['Up'])
    print(res)
...
#####################################################
    ...
    #result = search.breadth_first_graph_search(sg)    
    #result = search.astar_graph_search(sg)
    result=search.uniform_cost_search(sg)
    t1 = time.time()
    print(result)
    print(result.solution())
    print(result.state)
    print(result.parent)
    print(result.parent.state)
    ...
    # res=check_elem_action_seq(wh,['Right', 'Right','Down'])
    # print(res)
...
# #worker is moving to right and pushing a box 
#         if action=="Right":
#             #the worker location after moving Right
#             worker_location=(w_x+1,w_y) 
#             #if worker pushed a box
#             if (w_x+1,w_y) in boxes_location:
#                 #find the pushed box
#                 #update the box location after being pushed to Right
#                 i_box=boxes_location_list.index(worker_location)
#                 boxes_location_list[i_box]=(w_x+2,w_y)
           
           
                  

#         #worker is moving to left and pushing a box 
#         if action=="Left":
#             #the worker location after moving Left
#             worker_location=(w_x-1,w_y)
#             #if worker pushed a box
#             if (w_x-1,w_y) in boxes_location:
#                 #find the pushed box
#                 #the box location after being pushed to Left
#                 i_box=boxes_location_list.index(worker_location)
#                 boxes_location_list[i_box]=(w_x-2,w_y)
           
        
#         #worker is moving to down and pushing a box
#         if action=="Down":
#             #the worker location after moving Down
#             worker_location=(w_x,w_y+1)
#             if (w_x,w_y+1) in boxes_location:
#                 #find the pushed box
#                 #the box location after being pushed to Down
#                 i_box=boxes_location_list.index(worker_location)
#                 boxes_location_list[i_box]=(w_x,w_y+2)
            


#         #worker is moving to up and pushing a box
#         if action=="Up": 
#             #the worker location after moving Up
#             worker_location=(w_x,w_y-1)
#             if (w_x,w_y-1) in boxes_location:
#                 #find the pushed box 
#                 #the box location after being pushed to Up
#                 i_box=boxes_location_list.index(worker_location)
#                 boxes_location_list[i_box]=(w_x,w_y-2)
