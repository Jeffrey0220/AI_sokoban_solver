...
class SokobanPuzzle(search.Problem):
    ...
    def result(self, state, action):
        ...
        # make an dictionary of action offsets for easier caculate the new location tuples after actions 
    ...
    def h(self, n):
        """
        Heuristic for a current state to the goal state.
        
        Logics for computing the admissible heuristic cost:
            1. find the largest weight box.
            2. check if it's in the target position.
            3. if it's not in the target position, use Manhanttan distance to measure 
            the distance between this box and all the target position, and return the 
            minimal distance as the h(n) value.
            4. if the largest box is already in the target position, return 0 as the h(n) value.
        """
        ...
        # Choose the maximum distance to be the heuristic cost according to admissable heuristic requirement
        ...
        # Choose the minimal distance (distance of a box to its nearest target) 
        # to be the heuristic cost according to admissable heuristic requirement
        for i in range(len(self.boxes)):
            ...
            # if the box weighs nothing, only add the Manhattan distance to the nearest target to the h_costs
            if self.weights[i]==0:
...
def solve_weighted_sokoban(warehouse):
    ...
    #result=search.breadth_first_graph_search(sp)
    #result=search.uniform_cost_search(sp)
    if(result==None):
        return 'Impossible',None
    else:
